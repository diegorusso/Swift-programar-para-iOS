...
class Auto {
    ...
    func cambioDeVelocidad( ) -> ( actual : Int, velocidadEnCadena: String) {
        // Almacenamos la velocidad actual
        let velocidadActual = velocidad
        
        // Cambiamo de velocidad
        switch velocidad {
        case .Apagado:
        ...
        // Restituimos una tupla con la velocidad actual
        return (velocidadActual.rawValue, "\(velocidadActual)")
    }
}

// nueva instancia del Auto
var auto = Auto(velocidadInicial: Velocidades.Apagado)

// ciclo for
for i in 1...20{
    let data = auto.cambioDeVelocidad()
    print("\(data.0), \(data.1)")
}
